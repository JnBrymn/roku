#!/usr/bin/env python3
"""
Convert OBJ files downloaded from polyhedronisme to our format.

This script handles OBJ files from https://levskaya.github.io/polyhedronisme/

Usage:
    python3 convert_polyhedronisme.py input.obj output_name "Display Name"
    
Example:
    python3 convert_polyhedronisme.py ~/Downloads/polyhedron.obj expanded_truncated_icosahedron "Expanded Truncated Icosahedron"
"""
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 convert_polyhedronisme.py input.obj output_name [display_name]")
        print("\nExample:")
        print('  python3 convert_polyhedronisme.py ~/Downloads/polyhedron.obj expanded_truncated_icosahedron "Expanded Truncated Icosahedron"')
        sys.exit(1)
    
    input_path = Path(sys.argv[1])
    output_name = sys.argv[2]
    display_name = sys.argv[3] if len(sys.argv) > 3 else output_name.replace('_', ' ').title()
    
    # Determine output path
    script_dir = Path(__file__).parent
    project_root = script_dir.parent
    data_dir = project_root / 'public' / 'data'
    output_path = data_dir / f"{output_name}.txt"
    
    if not input_path.exists():
        print(f"Error: Input file not found: {input_path}")
        print(f"\nTip: Files from polyhedronisme usually download to ~/Downloads/")
        sys.exit(1)
    ...
    print(f"\nNext step: Add to lib/polyhedronUtils.ts:")
    print(f'  {{ file: \'/data/{output_name}.txt\', name: \'{display_name}\', slug: \'{output_name.replace("_", "-")}\' }},')
